...
var dgram = require('dgram'),
  ...
  debug = require('debug')('coap-dtls-server')
...
CoAPServer.prototype.listen = function(done) {
  ...
  if (this._options.dtls) {
    ...
    //debug(JSON.stringify(this._options.dtls));
    this._dtls_server = dtls.createServer(this._options.dtls, socket => {
      debug(`secure connection from ${socket.remoteAddress}:${socket.remotePort}`);
      //debug("...on socket:\n"+util.inspect(socket, {depth: 0}));
      //debug("...on socket:\n"+JSON.stringify(socket));

      socket.on('data', msg => {
        //debug('received:', msg.toString('utf8'));
        //var request = {
        //  raw:    msg,
      ...
      socket.once('close', () => {
        debug(`closing socket from ${socket.remoteAddress}:${socket.remotePort}`);
      });
    });
    ...
    this._dtls_server.on('listening', () => {
      ...
      debug(`dtls listening on ${addr.address}:${addr.port}`);
    });
    ...
    this._dtls_server.on('newSession', (sessionId, sessionData, callback) => {
      debug('*** new session callback ***', sessionId);
      process.nextTick(() => {  callback(); });
    });
    this._dtls_server.on('resumeSession', (sessionId, data, callback) => {
      debug('*** resume session callback ***', sessionId);
      process.nextTick(() => {  callback(null, null); });
    });

    this._dtls_server.listen(this._port, this._address, done);
  }
  else {
    // Non- DTLS workflow...
    ...
    debug("created non-secure server.\n");
  }
  return this
}
...
CoAPServer.prototype._handle = function(packet, rsinfo, sock_pass_in) {
  ...
  request = new IncomingMessage(packet, rsinfo, sock)
